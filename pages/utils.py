...
from .utils_segmentation import handle_meta_data_api_obj_det

# from vision.utils_segmentation import handle_meta_data_api_obj_det
...
def convert_to_dec_degrees(time_tupple, hemisphere):
    degrees = float(time_tupple[0])
    minutes = float(time_tupple[1])/60
    seconds = float(time_tupple[2])/3600
    hemisphere = hemisphere
    print(type(degrees), type(minutes), type(seconds))
    
    deciaml_value = degrees + minutes + seconds
    deciaml_value = round(deciaml_value, 4)
    print(f'degree {degrees} \t minutes {minutes}, seconds {seconds}, hemisphere {hemisphere}, deciaml_value {deciaml_value}')
...
def retrieve_bb(img, object_class, fields_to_search, client, index_name):
    ...
    if object_class in bb_field:
        ...
        for hit in bb_values['hits']['hits']:
            ...
            print((cur_img_name, new_bb_field), file=open('loop_w_out_idx.txt', 'a'))
            try:
        ...
        for idx in range(1, 20):
            new_bb_field_idx = new_bb_field.replace(cur_obj_name, f'{cur_obj_name}_{idx}')
            # print("old bb_field", bb_field)
            # print("new bb_field", new_bb_field)
            first_dot = new_bb_field_idx.find('.')
            cur_img_name = new_bb_field_idx[:first_dot]
            # print('cur_img_name = ', cur_img_name)
            query_for_bb_val = {
                "query": {
                    "match_all": {}
                },
                "_source": [new_bb_field_idx]
            ...
            for hit in bb_values['hits']['hits']:
                print((cur_img_name, new_bb_field_idx), file=open('loop_w_idx.txt', 'a'))
                try:
                    assert len(hit['_source'].keys()) != 0
                    # print('idxxxx', idx)
                    bb = hit['_source'][f'{cur_img_name}']['img_insights']['class_conf_bb_seg'][f'{cur_obj_name}_{idx}'][0]['bb']
                    bbboxes.append(bb)
                    # print("yes", bb)
                except AssertionError as e:
                    # print('assertion error', e)
...
def euclidean_dist(request, search_query, img_id_lat_long_bb_hitcount):
    ...
    for img_id, bb_lat_long_hitcount in img_id_lat_long_bb_hitcount.items():
        if bb_lat_long_hitcount['hit_count'] != 0:

            cur_img_id = img_id
            cur_img_name = bb_lat_long_hitcount['lat_long']['cur_img_name']
            # print(f"hit count {bb_lat_long_hitcount['hit_count']}")
            # print("Image name for l2dist", cur_img_name)
            cur_img_lat = float(bb_lat_long_hitcount['lat_long']['latitude'])
            cur_img_long = float(bb_lat_long_hitcount['lat_long']['longitude'])
            ...
            if distance < user_radius:
                ...
                close_imgs_id[cur_img_id].setdefault('bb', bb_lat_long_hitcount['bb'])
                close_imgs_id[cur_img_id].setdefault('img_name', cur_img_name)
                close_imgs_id[cur_img_id].setdefault('object', object_name)
        else:
            messages.error(request, "No document matching search conditions")
            
            
            
    return close_imgs_id

def draw_bb_on_s3_downlds(img_name, object_id, downloaded_imgs, name_bb_obj_name, img_dir):
    print(f"downloaded_imgs {downloaded_imgs} \t name_bb_obj_name {name_bb_obj_name} \t img_dir {img_dir}")
    ...
    cur_img_full_path = os.path.join(img_dir, img_frm_down_imgs)
    cur_img_bb = name_bb_obj_name['bb']
...
def extract_image_meta_data(image_metadata_dir, img, **kwargs):
    ...
    image_tags_context = {} #TODO: change the list to dictionary with the key as the image name
